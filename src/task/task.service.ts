...
export class TaskService {
  ...
  async create(createTaskDto: CreateTaskDto) {
    const { title, description, status, priority, creatorId, teamIds, dueDate, associationId } = createTaskDto
  ...
  async findAll(query: QueryTaskDto = { page: 1, limit: 10 }) {
    const { page = 1, limit = 10, title, status, priority, creatorId, associationId, sortBy = 'id', sortOrder = 'ASC' } = query ?? {}
    ...
    if (typeof priority !== 'undefined') where.priority = priority
    if (typeof creatorId !== 'undefined') where.creator = { id: creatorId } as User
    if (typeof associationId !== 'undefined') where.association = { id: associationId } as Association

    const [items, total] = await this.repo.findAndCount({
      relations: { creator: true, team: true, association: true },
      where,
  ...
  async findOne(id: string) {
    const task = await this.repo.findOne({ where: { id }, relations: { creator: true, team: true, association: true } })
  ...
  async update(id: string, updateTaskDto: UpdateTaskDto) {
    ...
    if (typeof (updateTaskDto as any).priority !== 'undefined') (task as any).priority = (updateTaskDto as any).priority
    ...
    if (typeof (updateTaskDto as any).associationId !== 'undefined') task.association = { id: (updateTaskDto as any).associationId } as Association
